...
export const CasePaperTab: React.FC<CasePaperTabProps> = ({
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200" onClick={() => setActiveMenuId(null)}>
      
      {/* ═════════════════════════════════════════════════════════════════════
          1. CASE PAPER SUMMARY BAR & CONTROLS
      ══════════════════════════════════════════════════════════════════════ */}
      <div className="bg-white rounded-2xl border border-outline-variant/15 p-4 sm:p-5 shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="relative flex-1 sm:w-56">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:outline-none focus:border-primary focus:bg-white transition-all shadow-2xs"
            />
          </div>
          ...
          <select
            ...
            className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:border-primary cursor-pointer shadow-2xs"
          >
          ...
          <button
            ...
            className="px-4 py-2 bg-primary hover:bg-primary/95 text-white rounded-xl text-xs font-bold transition-all shadow-xs active:scale-98 cursor-pointer flex items-center gap-1.5 shrink-0"
          >
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredEncounters.map((enc) => {
            ...
            return (
              <div
                key={enc.id}
                className="bg-white rounded-2xl border border-outline-variant/15 p-5 shadow-2xs hover:shadow-sm hover:border-primary/40 transition-all space-y-4 group"
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
                  ...
                  {/* Title & Status */}
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-primary/10 text-primary font-mono font-bold flex items-center justify-center text-sm border border-primary/20 shrink-0">
                      #{casePaperNumber}
                    </div>

                    <div>
                      <div className="flex items-center gap-2.5 flex-wrap">
                        <h3 className="text-base font-bold text-slate-900 font-sans group-hover:text-primary transition-colors">
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-auto relative">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-800 border border-blue-200 rounded-xl text-xs font-bold transition-colors cursor-pointer flex items-center gap-1 shadow-2xs"
                    ...
                    <a
                      ...
                      className="p-1.5 bg-[#dcfce7] hover:bg-green-200 text-green-900 rounded-xl transition-colors border border-green-300 shadow-2xs"
                    ...
                    <button
                      onClick={onPrint}
                      className="p-1.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-xl transition-colors cursor-pointer shadow-2xs"
                    ...
                    <button
                      ...
                      className="p-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl transition-colors cursor-pointer shadow-2xs"
                      title="More actions"
                    >
                    ...
                    {isMenuOpen && (
                      <div
                        className="absolute right-0 top-10 w-44 bg-white rounded-xl shadow-xl border border-slate-200 py-1.5 z-20 animate-in fade-in zoom-in-95 duration-150 text-xs"
                        onClick={(e) => e.stopPropagation()}
                      >
                        <button
                          onClick={() => {
                          ...
                          className="w-full px-3.5 py-2 text-left hover:bg-slate-50 text-slate-700 flex items-center gap-2 cursor-pointer font-medium"
                        >
                        ...
                        <button
                          ...
                          className="w-full px-3.5 py-2 text-left hover:bg-red-50 text-red-600 flex items-center gap-2 cursor-pointer font-medium border-t border-slate-100"
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      {complaints.map((c, idx) => (
                        <span
                          key={idx}
                          className="px-2.5 py-1 rounded-full bg-red-50 text-red-800 border border-red-200 font-semibold text-[11px]"
                        >
                          {c}
                        </span>
                      ))}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="p-2.5 bg-slate-50 border border-slate-200/80 rounded-xl font-medium text-slate-800 text-xs">
                      <strong>{enc.diagnosis || "Routine Oral Screening"}</strong>
                      {enc.notes && (
                ...
                <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    ...
                    <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 text-slate-800 rounded-xl font-semibold border border-slate-200">
                      <Activity className="w-3.5 h-3.5 text-primary" />
                      <span>{totalProcedures} {totalProcedures === 1 ? "Procedure" : "Procedures"} Logged</span>
                    </div>

                    {/* Fees / Billing Badge */}
                    {totalFees > 0 && (
                      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-900 rounded-xl font-bold border border-indigo-200 font-mono">
                        ...
                        {unbilledCount > 0 ? (
                          ...
                          <span className="text-[10px] bg-emerald-200 text-emerald-900 px-1.5 py-0.2 rounded font-sans">
                            Billed
                          </span>
                        )}
                      </div>
                    ...
                    {/* Prescription Badge */}
                    {hasPrescription && (
                      <div className="flex items-center gap-1 px-2.5 py-1.5 bg-emerald-50 text-emerald-800 rounded-xl text-[11px] font-bold border border-emerald-200">
                  ...
                  {/* Primary CTA: Open Case Paper */}
                  <button
                    onClick={() => handleOpenSession(enc.id)}
                    className="px-4 py-2 bg-slate-900 hover:bg-primary text-white rounded-xl text-xs font-bold transition-all shadow-2xs flex items-center justify-center gap-1.5 cursor-pointer self-stretch sm:self-auto group/btn"
                  >
